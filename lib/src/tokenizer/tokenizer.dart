...
extension ToList on Iterator<Token> {
  List<Token> toList() {
    List<Token> list = [];
    while (moveNext()) {
      list.add(current);
    ...
    while (true) {
      ...
      if (tryConsume('\n')) {
        ...
        _errorCollector.addError(
            startLine, startColumn, "  Comment started here.");
        if (content != null) stopRecording();
        break;
      }
    }
        ...
        if (tryConsumeOne(Letter())) {
          ...
          if (tryConsumeOne(Digit())) {
            ...
            if (_previous?.type == TokenType.identifier &&
              ...
              _errorCollector.addError(_line, _column - 2,
